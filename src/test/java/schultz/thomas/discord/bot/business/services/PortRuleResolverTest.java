...
import schultz.thomas.discord.bot.model.portforwarding.PortRuleResolution;
import schultz.thomas.discord.bot.model.portforwarding.Protocol;
...
class PortRuleResolverTest {
    ...
    @Mock
    private GamingServerService gamingServerService;
    ...
    void openPortsForOnlineServer() {
        givenServers(server("minecraft-ftb", ServerStatusEnum.ONLINE, port("tcp", 15007, null, null)));
    ...
    void closedPortsForOfflineServer() {
        givenServers(server("palworld-miam", ServerStatusEnum.OFFLINE, port("udp", 8211, null, null)));
    ...
    void overrideWinsForTargetedServerOnly() {
        givenServers(
                server("minecraft-ftb", ServerStatusEnum.OFFLINE, port("tcp", 15007, null, null)),
                server("palworld-miam", ServerStatusEnum.OFFLINE, port("udp", 8211, null, null))
        );
    ...
    void rejectsForbiddenPort() {
        givenServers(server("mauvaise-idee", ServerStatusEnum.ONLINE, port("tcp", 22, null, null)));
    ...
    void rejectsUnknownProtocol() {
        givenServers(server("bizarre", ServerStatusEnum.ONLINE, port("sctp", 1234, null, null)));
    ...
    void rejectsDuplicateWanPort() {
        givenServers(
                server("jeu-a", ServerStatusEnum.ONLINE, port("tcp", 25565, null, null)),
                server("jeu-b", ServerStatusEnum.ONLINE, port("tcp", 25565, null, null))
        );
    ...
    void ruleOverridesDefaults() {
        givenServers(server("special", ServerStatusEnum.ONLINE, port("tcp", 15007, 25565, "192.168.1.200")));
    ...
    void serverWithoutPortsProducesNothing() {
        givenServers(server("sans-ports", ServerStatusEnum.ONLINE));
    ...
    private void givenServers(GamingServerEntity... servers) {
        when(gamingServerService.getAllGameServerEntities()).thenReturn(List.of(servers));
    }

    private void givenNoServers() {
        when(gamingServerService.getAllGameServerEntities()).thenReturn(List.of());
    }
    ...
    private GamingServerEntity server(String identifier, ServerStatusEnum status, GamingServerPort... ports) {
        GamingServerEntity entity = new GamingServerEntity();
    ...
    private GamingServerPort port(String proto, Integer wanPort, Integer lanPort, String lanIp) {
        return new GamingServerPort(proto, wanPort, lanPort, lanIp);
    }
